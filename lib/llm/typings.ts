import { EventCallback } from '@boxyhq/saml-jackson';
...
export interface SAMLSSOConnectionWithEncodedMetadata
  extends SAMLSSOConnection {
  rawMetadata?: never;
...
export type DelConnectionsQuery =
  | (ClientIDQuery & { clientSecret: string })
  | TenantQuery;

export type GetConfigQuery = ClientIDQuery | Omit<TenantQuery, 'strategy'>;
export type DelConfigQuery =
  | (ClientIDQuery & { clientSecret: string })
...
export interface IOAuthController {
  authorize(
    body: OAuthReq
  ): Promise<{ redirect_url?: string; authorize_form?: string }>;
...
export interface ILogoutController {
  createRequest(
    body: SLORequestParams
  ): Promise<{ logoutUrl: string | null; logoutForm: string | null }>;
...
export type OAuthTokenReq =
  | OAuthTokenReqWithCodeVerifier
  | OAuthTokenReqWithCredentials;
...
export type SetupLinkCreatePayload =
  | (Pick<
      SetupLink,
      'name' | 'tenant' | 'product' | 'webhook_url' | 'webhook_secret'
    > & {
      service: 'dsync';
      regenerate?: boolean;
      expiryDays?: number;
