import { PII_POLICY_OPTIONS } from '../typings';
import { z } from 'zod';
...
export const llmApiKey = z
  ...
  .max(
    maxLengthPolicies.llmApiKey,
    `API key should be at most ${maxLengthPolicies.llmApiKey} characters`
  )
  .optional();

export const llmProvider = (providers) => {
  const LLM_PROVIDERS_KEYS = providers.map((provider) => provider.id);
  const maxLengthProvider = Math.max(
    ...LLM_PROVIDERS_KEYS.map((provider) => provider.length)
  );
  return z
    .string({
