...
const retraced = {
  ...
  externalUrl:
    process.env.RETRACED_EXTERNAL_URL || process.env.RETRACED_HOST_URL,
...
const db: DatabaseOption = {
  engine: process.env.DB_ENGINE
    ? <DatabaseEngine>process.env.DB_ENGINE
    : undefined,
  ...
  cleanupLimit: process.env.DB_CLEANUP_LIMIT
    ? Number(process.env.DB_CLEANUP_LIMIT)
    : undefined,
  encryptionKey: process.env.DB_ENCRYPTION_KEY,
  pageLimit: process.env.DB_PAGE_LIMIT
    ? Number(process.env.DB_PAGE_LIMIT)
    : undefined,
  ssl,
  dynamodb: {
    region: process.env.DB_DYNAMODB_REGION,
    readCapacityUnits: process.env.DB_DYNAMODB_RCUS
      ? Number(process.env.DB_DYNAMODB_RCUS)
      : undefined,
    writeCapacityUnits: process.env.DB_DYNAMODB_RCUS
      ? Number(process.env.DB_DYNAMODB_WCUS)
      : undefined,
  },
...
const jacksonOptions: JacksonOption = {
  ...
  preLoadedConnection:
    process.env.PRE_LOADED_CONNECTION || process.env.PRE_LOADED_CONFIG,
  ...
  openid: {
    ...
    requestProfileScope:
      process.env.OPENID_REQUEST_PROFILE_SCOPE === 'false' ? false : true,
    forwardOIDCParams:
      process.env.OPENID_REQUEST_FORWARD_PARAMS === 'true' ? true : false,
  },
  certs: {
  ...
  dsync: {
    ...
    providers: {
      google: {
        clientId:
          process.env.DSYNC_GOOGLE_CLIENT_ID ||
          process.env.GOOGLE_CLIENT_ID ||
          '',
        clientSecret:
          process.env.DSYNC_GOOGLE_CLIENT_SECRET ||
