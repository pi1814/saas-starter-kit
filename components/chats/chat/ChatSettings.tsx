...
export default function ChatSettings() {
  ...
  const [selectedProvider, setSelectedProvider] = useState<
    LLMProvidersOptionsType[number]['id'] | ''
  >('openai');
  ...
  useEffect(() => {
    if (errorLoadingConfigs || errorLoadingProviders || errorLoadingModels) {
      onError?.(
        errorLoadingConfigs?.message ||
          errorLoadingProviders?.message ||
          errorLoadingModels?.message
      );
    }
  ...
  return (
    <>
      <Card className="h-[88vh]">
        <Card.Body>
                ...
                return {
                  ...
                  cells: [
                    ...
                    },
                    {
                      wrap: true,
                      text:
                        config.models && typeof config.models !== 'string'
                          ? // .map((a: string) => {
          ...
          {(view === 'edit' || view === 'create') && (
            <form
              className="w-full"
              onSubmit={view === 'edit' ? updateLLMConfig : createLLMConfig}
            >
              <div className="flex flex-col gap-2">
                ...
                {((view === 'create' && !isChatWithPDFProvider) ||
                  (view === 'edit' &&
                    !selectedConfig.isChatWithPDFProvider)) && (
                  <>
                    <div>
                      <div className="label">
                        ...
                        {!isLoadingProviders ? (
                          <select
                            ...
                            onChange={(e) => {
                              setSelectedProvider(
                                e.target
                                  .value as LLMProvidersOptionsType[number]['id']
                              );
                              setSelectedModel([]);
                            }}
      ...
      <ConfirmationModal
        ...
        description={t('bui-chat-config-deletion-description')}
      ></ConfirmationModal>
    </>
  );
}
